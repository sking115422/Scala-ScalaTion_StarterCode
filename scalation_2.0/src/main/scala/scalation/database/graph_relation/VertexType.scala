...
case class VertexType (name: String, schema: VEC [String], eschema: VEC [String], verts: VEC [Vertex])
    ...
    def check: Boolean =
        for v <- verts do
            for pname <- v.prop.keys if ! (schema contains pname) do
                println (s"check: error pname = $pname not found in schema")
                return false
            end for
            for ename <- v.edge.keys if ! (eschema contains ename) do
                println (s"check: error ename = $ename not found in eschema")
                return false
            end for
        end for
        true
