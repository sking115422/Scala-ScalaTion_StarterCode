...
object Differential:
    ...
    inline def ∇ (f: FunctionV2S, x: VectorD): VectorD = gradient (f, x)
    ...
    def gradientD (d: FunctionsV, x: VectorD): VectorD =
        VectorD (for i <- x.indices yield d(i)(x))
    end gradientD

    inline def ∇* (d: FunctionsV, x: VectorD): VectorD = gradientD (d, x)
    ...
    inline def Ј (f: FunctionsV, x: VectorD): MatrixD = jacobian (f, x)       // want row-wise
...
@main def differentialTest (): Unit =
    ...
    val g  = (y: Double)  => 2 * (y - 3) * (y - 3)
    val f  = (x: VectorD) => 2 * (x(0) - 3) * (x(0) - 3) + (x(1) - 4) * (x(1) - 4) + x(0) * x(1)
