...
class ARIMA (y: VectorD, tt: VectorD = null, hparam: HyperParameter = ARIMA.hp)
    ...
    private val flaw   = flawf ("ARIMA")                         // flaw function
    private var cap    = 0                                       // max of p and q
    private var params = 0                                       // number of parameters estimated
    private val d      = hparam("d").toInt                       // the number of differences to take
    private val differenced = d > 0                              // flag indicating whether differencing will be applied
     ...
     *  Note, intercept/mean counts as a parameter for the time series.
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Train/fit an `ARIMA` model to the times series data.  Must call 'setPQ' first.
...
//      δ = stats.mu * (1 - φ.sum)
   ...
   //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
     ...
     */
    def nll (b: VectorD): Double =
        if (b.size != params + 1) flaw ("nll", "input parameter vector size incorrect")
     ...
     */
    override def forecast (t: Int = y.dim, h: Int = 1): VectorD =
...
//  val d = 0                                                  // apply d-th order differencing - no differencing
    ...
    for h <- 1 to 2 do                                         // forecasting horizon
        for p <- 0 to 4 do                                     // auto-regressive hyper-parameter settings
            for q <- 0 to 4 if p+q > 0 do                      // moving-average hyper-parameter settings
...
//                println (mod.report (mod.test (null, y)._2))

                val yfa = mod.forecastAll (h, y)
...
//  val noise = Uniform (-5, 5)
    ...
    // Build ARIMA(p, d, q) models

    mod.setPQ (VectorI (p, 0))
    mod.train (null, y)
    mod.testPred (y)

    mod.setPQ (VectorI (0, q))
    mod.train (null, y)
    mod.testPred (y)

    mod.setPQ (VectorI (p, q))
    mod.train (null, y)
    mod.testPred (y)
...
@main def aRIMATest3 (): Unit =
    ...
    val ma_f = mod.forecast (h = steps)
    println (s"$steps-step ahead forecasts using ${mod.modelName} model = $ma_f")

    mod.setPQ (VectorI (p, q))
    mod.train (null, y)
    mod.testPred (y)

    val arma_f = mod.forecast (h = steps)
    println (s"$steps-step ahead forecasts using ${mod.modelName} model = $arma_f")
...
@main def aRIMATest4 (): Unit =
    ...
    val m     = 50
    val d     = 1                                             // levels of differencing
    val steps = 2                                             // number of steps for the forecasts
    val sig2  = 10000.0
    ...
    banner (s"Build ARIMA(1, $d, 0) model")
    mod.setPQ (VectorI (1, 0))                                // train for ARIMA(1, d, 0) model
    mod.train (null, y)
    mod.testPred (y)

    for p <- 1 to 3 do
        banner (s"Build ARIMA($p, $d, $p) model")
        mod.setPQ (VectorI (p, p))                            // retrain for ARIMA(p, d, p) model
        mod.train (null, y)
        mod.testPred (y)
