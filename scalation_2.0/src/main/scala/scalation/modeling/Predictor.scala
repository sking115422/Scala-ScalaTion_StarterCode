...
trait Predictor (x: MatrixD, y: VectorD, protected var fname: Array [String], hparam: HyperParameter)
     ...
     */
    def buildModel (x_cols: MatrixD): Predictor = null
     ...
     */
    def selectFeatures (tech: Predictor.SelectionTech, idx_q: Int = QoF.rSqBar.ordinal, cross: Boolean = true):
                       (LinkedHashSet [Int], MatrixD) =
        tech match
        case Predictor.SelectionTech.Forward  => forwardSelAll (idx_q, cross)
        case Predictor.SelectionTech.Backward => backwardElimAll (idx_q, 1, cross)
        case Predictor.SelectionTech.Stepwise => stepRegressionAll (idx_q, cross)
        end match
    end selectFeatures
     ...
     */
    def swapVars (cols: LinkedHashSet [Int], out: Int, in: Int): BestStep =
...
//  import Example_BPressure._
    ...
    val fname_4  = Array ("modelyear")                                       // weight has highest correlation magnitute
    ...
    test (new TranRegression (ox, y, ox_fname, hp2, id _, id _), "id")       // 10 - id
    test (new TranRegression (ox, y, ox_fname, hp2, sqrt _, sq _), "sqrt")   // 11 - sqrt
    test (new TranRegression (ox, y, ox_fname, hp2, cbrt _, cb _), "cbrt")   // 12 - cbrt
