 ...
 *
 *  @title   Model Support: Proability and Entropy
 */
...
object Probability:
     ...
     *  The elements of the matrix must be non-negatmive and add to one.
     ...
     */
    def frequency (x: VectorI, y: VectorI, k: Int, vl: Int, idx_ : VectorI): (Double, VectorI) =
        val idx = if idx_ == null then VectorI.range (0, y.dim) else idx_
        val nu  = new VectorI (k)                                 // frequency counts
        var cnt = 0                                               // count for branch value
        for i <- idx if x(i) == vl do { nu(y(i)) += 1; cnt += 1 }
        debug ("frequency", s"k = $k, vl = $vl, nu = $nu")
     ...
     */
    def frequency (x: VectorD, y: VectorI, k: Int, vl: Int, idx_ : VectorI,
                   cont: Boolean, thres: Double): (Double, VectorI) =
        val idx = if idx_ == null then VectorI.range (0, y.dim) else idx_
        val nu  = new VectorI (k)                                 // frequency counts
        ...
        debug ("frequency", s"k = $k, vl = $vl, cont = $cont, thres = $thres, nu = $nu")
        (cnt.toDouble / idx.size, nu)                             // return fraction and frequency vector
    end frequency
     ...
     *  @param thres  the splitting threshold for features/variables treated as continuous
     */
    def count (x: VectorD, vl: Int, idx_ : VectorI, cont: Boolean, thres: Double): Int =
        val idx = if idx_ == null then VectorI.range (0, x.dim) else idx_
        var cnt = 0                                               // count for the value branch
        if cont then
            if vl == 0 then
                for i <- idx if x(i) <= thres do cnt += 1
            else
                for i <- idx if x(i) >  thres do cnt += 1
            end if
        else
            for i <- idx if x(i) == vl do cnt += 1
        end if
        cnt
    end count
     ...
     *  @param px  the probability vector
     */
    def logProb (px: VectorD): VectorD = px.map (- log2 (_))
 ...
 *  Plot probability and log-probability entropy and normalized entropy for binomial distributions.
...
@main def probabilityTest3 (): Unit =
    ...
    for n <- 2 to 16 do
        ...
        val lp = logProb (pp)                                  // log-probability vector
 ...
 *  It computes entropy, relative entropy and cross entropy and verifies thst
 ...
 *  @see ocw.mit.edu/courses/mathematics/18-05-introduction-to-probability-and-statistics-spring-2014/readings/MIT18_05S14_Reading7a.pdf
