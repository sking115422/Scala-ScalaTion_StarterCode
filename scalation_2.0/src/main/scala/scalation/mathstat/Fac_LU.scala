...
object Fac_LU:
    ...
    def test (a: MatrixD, b: VectorD): Unit =
        ...
        if a.dim == a.dim2 then
            ...
            println (s"ai = $ai")
            println (s"aai = $aai")
            assert (aai == MatrixD.eye (n, n))             // ensure A * A^-1 = I
            assert (ai == inverse (a))                     // ensure both inverses are the same
        end if
...
@main def fac_LUTest (): Unit =
    ...
    test (a, b)
    println (s"det (a)     = ${det (a)}")
    println (s"inverse (a) = ${inverse (a)}")
