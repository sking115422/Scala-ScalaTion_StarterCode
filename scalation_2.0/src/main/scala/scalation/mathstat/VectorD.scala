...
/** Top-level type definition for functions mapping vector `VectorD` to scalar `Double`
 */
...
class VectorD (val dim: Int,
     ...
     */
    def apply (r: Range): VectorD = new VectorD (r.end - r.start, v.slice (r.start, r.end))
     ...
     */
    def map (f: Double => Double): VectorD = new VectorD (v.size, v.map (f))
    ...
    //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    /** Determine whether this vector and y are nearly equal.
     */
    def =~ (y: VectorD): Boolean =
        if dim != y.dim then return false
        for i <- indices if ! (v(i) =~ y.v(i)) do return false
        true
    end =~
     ...
     */
    def median (k: Int = (dim+1)/2): Double = median (Array.range (0, dim), 0, dim-1, k)
     ...
     */
    def cov (y: IndexedSeq [Double]): Double  = ((this dot y) - sum * y.sum / nd) / (nd-1)
    def cov_ (y: IndexedSeq [Double]): Double = ((this dot y) - sum * y.sum / nd) / nd
    ...
    def acorr_ (k: Int = 1): Double =
        ...
        var sum = 0.0
        for i <- 0 until n do sum += (v(i) - mu._1) * (v(i+k) - mu._2)
        (sum / n) / (math.sqrt (vr._1) * math.sqrt (vr._2))
    end acorr_
...
object VectorD:
    ...
    def apply (x: String, xs: String*): VectorD =
        ...
        for i <- y.indices do
            try y(i) = if i == 0 then x.toDouble else xs(i-1).toDouble
            catch { case _ : NumberFormatException | _ : ClassCastException => y(i) = -0.0 }
        end for
        y
    end apply
     ...
     */
    def fromStrings (xs: IndexedSeq [String]): VectorD = VectorD (xs.map (_.toDouble))
