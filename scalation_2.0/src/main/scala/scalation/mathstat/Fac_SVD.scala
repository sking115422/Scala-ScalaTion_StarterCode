...
// FIX: make naming of matrices consistent
    ...
    def factor123 (): FactorType =
        ...
        def cancellation (l : Int, k : Int): Unit =
            ...
            for j <- l to k do                            // each column l to k
                f     = s * e(j)                          // sine * e(j)
                e(j) *= c                                 // cosine * e(j)

                if abs (f) <= eps then return             // f near zero => return & test f convergence
