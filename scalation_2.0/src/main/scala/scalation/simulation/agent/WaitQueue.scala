 ...
 *
 *  @title   WaitQueue Provides a Place for Entities/SimAgents to Wait
 */
...
class WaitQueue (name: String, director: Model, cap: Int = Int.MaxValue,
     ...
     */
    def ping (): Unit =
        if ! que.isEmpty then director.schedule (que.dequeue (), director.clock)
    end ping
 ...
 */
object WaitQueue
       extends VertexType ("WaitQueue", VEC ("type"), color = sandybrown,
                           shape = Rectangle ()):

    Model.add (WaitQueue)

    private val debug = debugf ("WaitQueue_", true)            // debug function
