...
class GradientDescent (f: FunctionV2S, exactLS: Boolean = true)
    ...
    private var given_ = false                                // default: functions for partials are not given
     ...
     */
    def setDerivatives (partials: Array [FunctionV2S]): Unit = { df = partials; given_ = true } 
    ...
    def solve (x0: VectorD, step: Double = STEP, toler: Double = EPSILON): FuncVec =
        ...
        var dir  = if given_ then -gradientD (df, x)          // initial direction is -gradient: use partials
        ...
        for k <- 1 to MAX_ITER if down && dist > toler && dir.normSq > toler do
            ...
            dir = if given_ then -gradientD (df, y)           // next search direction: use partials
...
@main def gradientDescentTest (): Unit =
    ...
    banner ("Minimize (with partials): (x_0 - 2)^2 + (x_1 - 3)^2 + 1") 
    def df_dx0 (x: VectorD): Double = 2*x(0) - 6
    def df_dx1 (x: VectorD): Double = 2*x(1) - 8

    optimizer.setDerivatives (Array [FunctionV2S] (df_dx0, df_dx1))
