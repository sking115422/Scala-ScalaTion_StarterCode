...
object DormandPrince
    ...
    def integrate2 (f: Derivative, y0: Double, t: Double, hmin: Double, hmax: Double,
        ...
        for i <- 1 to maxSteps do
            ...
            if delta <= .1 then       h *= .1
            else if delta >= 4.0 then h *= 4.0
            else h *= delta
 
            if h > hmax then h = hmax
 
            if ti >= t then         return y
            else if ti + h > t then h = t - ti
            else if h < hmin then   return y
        end for
    ...
    def integrateVV (f: Array [DerivativeV], y0: VectorD, t: Double,
        ...
        for i <- 1 to maxSteps do
            for j <- y.indices do k1(j) = f(j)(ti,          y)
            for j <- y.indices do k2(j) = f(j)(ti + c2 * h, y + (k1*a21) * h)
            for j <- y.indices do k3(j) = f(j)(ti + c3 * h, y + (k1*a31 + k2*a32) * h)
            for j <- y.indices do k4(j) = f(j)(ti + c4 * h, y + (k1*a41 + k2*a42 + k3*a43) * h)
            for j <- y.indices do k5(j) = f(j)(ti + c5 * h, y + (k1*a51 + k2*a52 + k3*a53 + k4*a54) * h)
            for j <- y.indices do k6(j) = f(j)(ti +      h, y + (k1*a61 + k2*a62 + k3*a63 + k4*a64 + k5*a65) * h)
            for j <- y.indices do k7(j) = f(j)(ti +      h, y + (k1*a71 + k2*a72 + k3*a73 + k4*a74 + k5*a75 + k6*a76) * h)
            ...
            if error < tol then
                ti += h
                for j <- y.indices do y(j) += h * (b1*k1(j) + b3*k3(j) + b4*k4(j) + b5*k5(j) + b6*k6(j))
            end if

            if delta <= 0.1 then      h *= .1
            else if delta >= 4.0 then h *= 4.0
            else h *= delta
 ...
 *  This test is for non-stiff equations.  Compare ode45 with ode23s.
...
//:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
/** The `dormandPrincekTest2` main function is used to test the `DormandPrince` object.
 *  This test is for stiff equations.  Compare ode45 with ode23s.
 *  > runMain scalation.dynamics.dormandPrincekTest2
 */
@main def dormandPrincekTest2 (): Unit =

    println ("dormandPriceTest2 not yet implemented")          // FIX - implement

end dormandPrincekTest2
